...
public class ServerMapLoader extends AsynchronousAssetLoader<TiledMap, TmxMapLoader.Parameters> {
	 ...
	 * 
	 * @param resolver */
	public ServerMapLoader (FileHandleResolver resolver) {
		super(resolver);
	}
	...
	public TiledMap load (String fileName, TmxMapLoader.Parameters parameters) {
		try {
			...
			FileHandle tmxFile =  new FileHandle(new File(fileName));
	...
	protected void loadTileSet (TiledMap map, Element element, FileHandle tmxFile) {
		if (element.getName().equals("tileset")) {
            ...
            //only put in one marker tile per tileset
            {
                //since the below loop also declares a tile variable
//                TextureRegion tileRegion = new TextureRegion((Texture)null, 0, 0, Game.PIXELS_PER_TILE, Game.PIXELS_PER_TILE);
	...
	protected void loadTileLayer (TiledMap map, Element element) {
		if (element.getName().equals("layer")) {
            ...
            System.out.println("loading tile layer " + name);
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
                    ...
                    //System.out.printf("%d,%d raw tile ID: %d\n", x,y,id);
                    
                    //using tile ID 1 for all non-empty tiles
					//if the raw tile ID is non-zero, put a cell here
                    TiledMapTile tile = tilesets.getTile(1);
					if (id != 0) {
						Cell cell = createTileLayerCell(flipHorizontally, flipVertically, flipDiagonally);
						cell.setTile(tile);
						layer.setCell(x, height - 1 - y, cell);
                        //System.out.printf("layer %s: set tile %d,%d\n", layer.getName(), x, height - 1 - y);
					}
				}
			}
